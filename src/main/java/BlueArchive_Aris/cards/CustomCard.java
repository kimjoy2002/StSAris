...
public abstract class CustomCard extends AbstractDynamicCard {
    public static class ability {
        ...
        public String getDescription() {
            ...
            if(level == 1) {
                description_ = description_.replace("(s)", "");
            }
            else if(level > 1) {
                description_ = description_.replace("(s)", "s");
            }
    ...
    public static void initializeCustomCard(CustomCard card, List<ability> currentAbility_) {
    ...
    public void use(AbstractPlayer p, AbstractMonster m, List<ability> currentAbility_) {
        for (ability ability_ : currentAbility_) {
    ...
    public void makeDescription(List<ability> currentAbility_) {
            this.rawDescription = "";
    ...
    public void triggerWhenDrawn(List<ability> currentAbility_) {
        for (ability ability_ : currentAbility_) {
    ...
    static public AbstractCard makeCustomCard(CustomCard card, int cost, int value, CardType type, List<ability> currentAbility_, boolean isCreated) {
    ...
    static {
        cardStrings = CardCrawlGame.languagePack.getCardStrings(DefaultMod.makeID(CustomGameCard.class.getSimpleName()));
        TEXT = cardStrings.EXTENDED_DESCRIPTION;

        abilityList = new ArrayList<>(Arrays.asList(
                new ability("innate", 20, 10, 1, 1, TEXT[0]),
                ...
                new ability("deal", 0, 8, 1, 100, TEXT[5]), //기본으로 들어가야함
                new ability("dealAll", 100, 25, 1, 1, TEXT[6]),   //광역
                new ability("dealRepeat", 100, (ability base, List<ability> list) ->
                {
                ...
                new ability("dealRandomly", 20, (ability base, List<ability> list) ->
                {
                ...
                new ability("voidWhenDrawn", 3, -80, 1, 1, TEXT[70]),
        ...
        uncreatableAbliity = new ArrayList<>(Arrays.asList("innate", "heal", "potion", "gold", "maxhpUp", "upgradeEternal", "voidWhenDrawn"));
